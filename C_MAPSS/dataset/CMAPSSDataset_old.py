...
class CMAPSSDataset_old(Dataset):
    """
    ...
    def get_datasets(
            ...
            percent_of_broken_data: float=0.0
    ):
        """
        ...
        # print
        train_dataset = CMAPSSDataset_old(
            train_df,
            init=False,
            **dataset_kwargs
        )
        ...
        test_dataset = CMAPSSDataset_old(
            test_df,
            init=False,
            **dataset_kwargs
        )
        ...
        if valid_df is not None:
            ...
            valid_dataset = CMAPSSDataset_old(
                valid_df,
                init=False,
                **dataset_kwargs
            )
        ...
        if cluster_operations:
            CMAPSSDataset_old.clustering_operations(dataset_list)

        if norm_type:
            norm_params = CMAPSSDataset_old.gen_norm_params(dataset_list, norm_type, norm_by_operations)
            for dataset in dataset_list:
    ...
    @staticmethod
    def get_data_loaders(loader_kwargs, train_kwargs=None, test_kwargs=None, valid_kwargs=None,
        ...
        train_dataset, test_dataset, valid_dataset = CMAPSSDataset_old.get_datasets(**dataset_kwargs)
    ...
    def get_data_for_ssl_pct(
            ...
            use_max_rul_on_test=False,
            use_max_rul_on_valid=True
    ) -> tuple['CMAPSSDataset_old', 'CMAPSSDataset_old', 'CMAPSSDataset_old']:
        train_dataset, test_dataset, valid_dataset = CMAPSSDataset_old.get_datasets(
    ...
    def __init__(
        ...
        self.df.columns = CMAPSSDataset_old.DATASET_COLS
        # print('init', self.df)
        # sequence_len
        ...
        if include_cols is not None:
            ...
            self.feature_cols = CMAPSSDataset_old.OPERATION_COLS + CMAPSSDataset_old.SENSOR_COLS
        ...
        if self.init:
            if self.cluster_operations:
                CMAPSSDataset_old.clustering_operations([self])
    ...
    def normalization(self):
        ...
        if self.norm_params is None:
            self.norm_params = CMAPSSDataset_old.gen_norm_params([self], norm_type, norm_by_operations)
        norm_params = self.norm_params
        if norm_type == '0-1':
            min_norm, max_norm = 0, 1
            self.df = CMAPSSDataset_old.min_max_normalization(df, norm_cols, norm_params, min_norm, max_norm)
        if norm_type == '-1-1':
            min_norm, max_norm = -1, 1
            self.df = CMAPSSDataset_old.min_max_normalization(df, norm_cols, norm_params, min_norm, max_norm)
        if norm_type == 'z-score':
            self.df = CMAPSSDataset_old.z_score_normalization(df, norm_cols, norm_params)
