...
def f_construct_dataset(df, feat_list):
    ...
    for i, tmp_id in enumerate(id_list):
        ...
        pat_info[i,2] = tmp['label'].iloc[0]      #cause
        pat_info[i,1] = tmp['tte'].iloc[0]         #time_to_event
        pat_info[i,0] = tmp['id'].iloc[0]

        data[i, :int(pat_info[i, 4]), 1:]  = tmp[feat_list].to_numpy()
