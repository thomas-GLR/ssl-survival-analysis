...
NOTE: this script targets a private cystic-fibrosis dataset and CANNOT be run as-is. It calls
      impt.import_dataset_cysfib_ver7_missing(), which does not exist in import_data.py. The TensorFlow
      2.x conversion below (and the realignment of the model call signatures) is therefore best-effort
      and unverified -- only main.py / tutorial.ipynb have been run end to end.
'''
_EPSILON = 1e-08
...
def div(x, y):
    return tf.math.divide(x, (y + 1e-8))
...
def f_get_prediction_v5(model, data, data_org, data_mi, time, label, mask2, pred_horizon):
    ...
    return model.predict(new_data, new_data_mi), new_time, new_label, last_meas
...
def get_valid_performance(in_parser, out_itr, MAX_VALUE = -99, OUT_ITERATION=5, seed=1234):
    ...
    network_settings            = { 'h_dim_RNN'         : in_parser['h_dim_RNN'],
                                    ...
                                    'initial_W'         : initial_W,
                                    'reg_W'             : in_parser.get('reg_W', 1e-5),
                                    'reg_W_out'         : in_parser.get('reg_W_out', 0.) }
    ...
    model = Model_Longitudinal_Attention("FHT_Landmarking", input_dims, network_settings)
    ...
    for itr in range(iteration):
        if stop_flag > 5: #for faster early stopping
            ...
            x_mb, x_org_mb, x_mi_mb, k_mb, t_mb, m1_mb, m2_mb, m3_mb, m4_mb, m5_mb = f_get_minibatch(mb_size, tr_data, tr_data_org, tr_data_mi, tr_label, tr_time, tr_mask1, tr_mask2, tr_mask3, tr_mask4, tr_mask5)
            DATA = (x_mb, k_mb, t_mb)
            MASK = (m3_mb, m4_mb, m5_mb)   #fc_mask1, fc_mask2, fc_mask3
            MISSING = (x_mi_mb)
            ...
            if (itr+1)%1000 == 0:
                if valid_mode == 'ON':
                    for p, p_time in enumerate(pred_time):
                        ...
                        pred, tmp_time, tmp_label, _ = f_get_prediction_v5(model, va_data, va_data_org, va_data_mi, va_time, va_label, va_mask2, pred_horizon)
                    ...
                    if tmp_valid >  max_valid:
                        stop_flag = 0
                        max_valid = tmp_valid
                        ckpt.write(file_path + '/models/model_itr_' + str(out_itr))
                        print( 'updated.... average c-index = ' + str('%.4f' %(tmp_valid)))

                        if max_valid > MAX_VALUE:
                            ckpt.write(file_path_final + '/models/model_itr_' + str(out_itr))
                    else:
                        stop_flag += 1
