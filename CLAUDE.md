...
`data/C_MAPSS/` holds the turbofan `train_/test_/RUL_FDxxx` files; `data/Scania_component_X/` holds the Scania readouts/TTE/specifications CSVs (plus a `scania_cache/`). `CMAPSSLoader.get_datasets()` produces train/valid/test `CMAPSSDataset`s with windowing, normalization, operating-condition clustering, and censoring. `ScaniaDataModule` splits **by vehicle** from the train files only (the `validation_*` files are intentionally ignored) and caches processed splits. The one exception is `ScaniaDataModule.build_challenge_dataset()`, which reads the official held-out `test_operational_readouts.csv` through the *same* `_engineer_features` and the *same* train-fitted normalization params, `only_final=True` (one window per vehicle), and is never cached — it feeds the challenge evaluation below.
...
- **Two different test sets, and they measure different things.** The `test_*` columns above come from an **internal** split — a slice of the *training* vehicles, restricted to uncensored (failure) rows. On top of that, every Scania run is now also scored on the **official Component X held-out sets** (`scania/challenge.py`, `split="test"` by default, `split="validation"` additionally): `test_operational_readouts.csv` + `test_labels.csv` (5045 vehicles) and, run right after it, the disjoint `validation_operational_readouts.csv` + `validation_labels.csv` (5046 vehicles) — same ~97% class 0 imbalance, zero vehicle-id overlap between the two. Both are mostly *healthy* vehicles, which the models never see a label for during training, so these numbers are not comparable to the internal `test_*` columns and are usually far worse. The ground truth there is a class, not an RUL, so predictions are binned with `rul_to_class` — verified to be the official binning, since applying it to `length_of_study_time_step - max(time_step)` (censored → class 0) reproduces `train_label.csv` for all 23550 training vehicles. Metrics: accuracy, macro-F1 (always over all 5 `CLASS_LABELS`, or the average is not comparable between runs) and the same Scania cost, via `scania_classification_metrics`. Outputs, in the run's normal results folder: `<mv>-challenge-scania.csv` / `<mv>-challenge-validation-scania.csv` (one row per predictor) and `predictions_<mv>_challenge_<name>_scania.csv` / `predictions_<mv>_challenge_validation_<name>_scania.csv` (`vehicle_id, target_class, predicted_class, predicted_rul`). Hooked into every training tail — `utils_simple_lightning_model`, `save_ensemble_outputs` (v1 + v2 + the v2 HPO sweep), `utils_coprog`, `utils_dynamic_deephit` and `utils_random_survival_forest` (row-wise, via `ScikitDataset.from_scania_challenge`) — each calling `run_challenge_evaluation`/`run_challenge_evaluation_rowwise` twice, once per split. `utils_dynamic_deephit`'s `predict_fn` closure additionally needs the split's own `challenge_set` (for `time_step_array`, used to digitize measurement time bins), not just its features, so it takes an explicit `split` argument re-selecting `build_challenge_dataset()`/`build_validation_challenge_dataset()` per call — a trap for any other model whose challenge prediction reads something off `data_module` beyond the `features` tensor it's handed. It is **always on, never raises, and never selects anything**: it runs after all other artifacts are written and degrades to a printed `[Scania][challenge] skipped:` when the held-out CSVs for that split are absent (independently of the other split). Align predictions to labels on `vehicle_id`, never positionally — `ScaniaDataset._gen_sequence` emits long vehicles before short ones, so window order is not input order.
